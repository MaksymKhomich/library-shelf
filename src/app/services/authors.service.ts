...
export class AuthorsService {
  authorsList: Array<Author> = JSON.parse(localStorage['authorsList'])
  ...
  deleteAuthor(id: number): Observable<any> {
    return of({}).pipe(map(() => {
      this.authorsList.splice(this.authorsList.findIndex((item:any) => item.id === id), 1)
      localStorage.setItem('authorsList', JSON.stringify(this.authorsList));
    }));
  }
